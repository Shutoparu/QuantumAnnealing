 ...
 */
__global__ void initMomentum(float* arr, float* spin, int size) {
    int id = blockDim.x * blockIdx.x + threadIdx.x;
    if (id < size - 1) {
        curandState state;
        curand_init(clock64(), id, 0, &state);
        arr[id] = curand_uniform(&state) * (spin[id] < 0 ? -1 : 1);
    }
    if (id == size - 1) {
        arr[id] = 0;
    }
...
void iterate(float* spin, float* qubo, int dim, int window, int maxStep) {
    ...
    initMomentum << <blocks, threads >> > (momentum, spin_dev, dim);
    ...
    if (window == 0) {
        ...
        for (int i = 0; i < maxStep; i++) {
            ...
            if (i % window == 0) {
                ...
                if (i != 0) {
                    // printf("--Diff spin = %d--\n", dim - sameSpin(sample[i / window], sample[i / window - 1], dim));
