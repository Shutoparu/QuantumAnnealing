...
if __name__ == '__main__':
    ...
    init_spin = []
    for i in range(np.shape(init_bin)[0]):
        init_spin.append(2*np.random.rand()-1)
    init_spin[-1] = 1
    init_spin = np.array(init_spin)

    sbm = SBM(Q[0], init_spin, maxStep=100000, window=0)
    ...
    spin2bin2 = list()
    for i in spin:
        spin2bin2.append(1 if i > 0 else 0)
