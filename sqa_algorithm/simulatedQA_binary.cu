 ...
 */
float energy(int* b, float* Q, int dim) {
    float* temp;
    temp = (float*)malloc(sizeof(float) * dim);
    float sum = 0;
    for (int i = 0; i < dim; i++) {
        for (int j = 0; j < dim; j++) {
            temp[i] += b[j] * Q[i * dim + j];
        }
    }
    for (int i = 0; i < dim; i++) {
        sum += temp[i] * b[i];
 ...
 * @param binIdx the index of the checked bit in the trotter block
 ...
 */
__global__ void calculate(int* trotterBlock, int binIdx, int dim, int trotterNum, int beta, int seed) {
    ...
    if (index == 0) {
        ...
        // check flip
        if (tex1Dfetch(s_text, binIdx) == 0) {
            flipped = 1;
        }

        for (int n = 0; n < dim; n++) {
            if (n == binIdx % dim && flipped == 1) {
                delta_E += tex1Dfetch(Q_text, (binIdx % dim) * dim + n); // time consuming
            } else {
                delta_E += tex1Dfetch(s_text, dim * (binIdx % dim) + n) * tex1Dfetch(Q_text, (binIdx % dim) * dim + n); // time consuming
            }
        }

        if (flipped != 0) {
            delta_E = 2 * delta_E - tex1Dfetch(Q_text, (binIdx % dim) * dim + (binIdx % dim));
        } else {
            delta_E = -2 * delta_E + tex1Dfetch(Q_text, (binIdx % dim) * dim + (binIdx % dim));
        }

        // calculate with previous trotter
        if (binIdx - dim >= 0) {
            delta_E -= tex1Dfetch(s_text, binIdx - dim) * K;
        } else {
            delta_E -= tex1Dfetch(s_text, dim * (trotterNum - 1) + binIdx % dim) * K;
        }

        // calculate with following trotter
        if (binIdx + dim < dim * trotterNum) {
            delta_E += tex1Dfetch(s_text, binIdx + dim) * K;
        } else {
            delta_E += tex1Dfetch(s_text, binIdx % dim) * K;
        }
...
extern "C"
{
    float simulatedQA(int* b, float* Q, int dim, int trotterNum, int totalSweeps);
}
...
float simulatedQA(int* b, float* Q, int dim, int trotterNum, int totalSweeps) {
    ...
    for (int i = 0; i < trotterNum; i++) {
        cudaErr(cudaMemcpy(&trotterBlock[i * dim], b, dim * sizeof(int), cudaMemcpyHostToDevice));
    }
    ...
    for (int sweep = 0; sweep < totalSweeps; sweep++) {
        ...
        cudaMemcpy(b, &trotterBlock[dim * (trotterNum - 1)], dim * sizeof(int), cudaMemcpyDeviceToHost); // dim * (trotterNum - 1)
        for (int i = 0; i < dim; i++) {
            printf("%d", b[i]);
        }
        printf("\n");
    ...
    cudaMemcpy(b, &trotterBlock[0], dim * sizeof(int), cudaMemcpyDeviceToHost); // dim * (trotterNum - 1)
